...
class DashboardController extends Controller
{
    ...
    public function getMetrics()
    {
        ...
        // Use /opt/webserver instead of / to avoid open_basedir restriction
    ...
    /**
     * Get historical metrics for charts (SPRINT 8)
     */
    public function getHistoricalMetrics($hours = 24)
    {
        $metrics = \App\Models\MetricsHistory::where('created_at', '>=', now()->subHours($hours))
            ->orderBy('created_at', 'asc')
            ->get();
        
        return [
            'labels' => $metrics->pluck('created_at')->map(fn($dt) => $dt->format('H:i'))->toArray(),
    ...
    /**
     * API endpoint for historical metrics (SPRINT 8)
     */
