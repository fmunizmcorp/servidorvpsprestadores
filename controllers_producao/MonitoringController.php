...
class MonitoringController extends Controller
{
    ...
    {
        $rawMetrics = $this->getSystemMetrics();
        $services = $this->getServicesStatus();
        
        // Simplify metrics for view
        $metrics = [
            'cpu' => $rawMetrics['cpu']['usage'] ?? 0,
            'memory' => $rawMetrics['memory']['usage'] ?? 0,
            'disk' => $rawMetrics['disk']['usage'] ?? 0,
    ...
    private function getSystemMetrics()
    {
        ...
        return [
            'cpu' => [
                'usage' => $cpuUsage,
                'load' => implode(', ', array_map(function($v) { return number_format($v, 2); }, $cpuLoad))
            ],
            'memory' => [
                'usage' => $memUsage
            ],
