...
class EmailController extends Controller
{
    ...
    /**
     * SPRINT 3: Show edit account form
     */
    ...
    /**
     * SPRINT 3: Update existing email account
     */
    public function updateAccount(Request $request, $id)
    {
        ...
        $oldEmail = $emailAccount->email;
        $newEmail = $request->username . '@' . $request->domain;
        
        // Check if new email already exists (excluding current account)
        if ($newEmail !== $oldEmail && EmailAccount::where('email', $newEmail)->exists()) {
            return redirect()->route('email.accounts', ['domain' => $request->domain])
                ->with('error', 'Email account already exists');
        }

        // If email changed, need to rename system mailbox
        if ($newEmail !== $oldEmail) {
            $script = "{$this->scriptsPath}/rename-email-account.sh";
            if (file_exists($script)) {
                $output = [];
                $returnVar = 0;
                $command = "sudo bash {$script} {$oldEmail} {$newEmail} 2>&1";
                exec($command, $output, $returnVar);
                
                if ($returnVar !== 0) {
                    \Log::error("Failed to rename email account via script", [
                        'old_email' => $oldEmail,
                        'new_email' => $newEmail,
                        'output' => $output,
                        'return_code' => $returnVar
                    ...
                    return redirect()->route('email.accounts', ['domain' => $request->domain])
                        ->with('error', 'Failed to rename email account in system');
                }
            }
        }
        ...
        $emailAccount->domain = $request->domain;
        $emailAccount->quota = $request->quota;
        ...
        if ($request->filled('password')) {
            $script = "{$this->scriptsPath}/change-email-password.sh";
            if (file_exists($script)) {
                exec("sudo bash {$script} {$newEmail} {$request->password} 2>&1", $output, $returnVar);
            }
        }
