...
class SitesController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        try {
            ...
            \Log::info('SPRINT_RECOVERY: Script output', ['output' => substr($output ?? '', 0, 500)]);
            
            // RECOVERY FIX: Instead of relying on script output, verify physical creation
            // The script may take time and output might be truncated/buffered
            sleep(1); // Give script time to complete
            
            $sitePath = "/opt/webserver/sites/$siteName";
            $poolFile = "/etc/php/$phpVersion/fpm/pool.d/$siteName.conf";
            
            if (!is_dir($sitePath)) {
                \Log::error('SPRINT_RECOVERY: Site directory not created', ['path' => $sitePath]);
                throw new \Exception("Site directory was not created. Check script logs for errors.");
            }
            
            if (!file_exists($poolFile)) {
                \Log::error('SPRINT_RECOVERY: PHP-FPM pool not created', ['path' => $poolFile]);
                throw new \Exception("PHP-FPM pool was not created. Check script logs for errors.");
            }
            
            \Log::info('SPRINT_RECOVERY: Site physically created, persisting to database');
    ...
    /**
     * Renew SSL certificate (SPRINT 11)
     */
    public function renewSSL($siteName)
    {
        try {
            $site = $this->getSiteDetails($siteName);
            ...
            $command = "certbot renew --cert-name $domain --force-renewal --nginx 2>&1";
            $output = shell_exec($command);
            
            // Check if successful
            if (strpos($output, 'Successfully') !== false || strpos($output, 'renewed') !== false) {
                return redirect()->route('sites.ssl', $siteName)
                    ->with('success', 'SSL certificate renewed successfully!');
            } else {
                throw new \Exception("Certbot renew failed: $output");
            }
            
        } catch (\Exception $e) {
            return redirect()->back()
                ->with('error', 'Failed to renew SSL: ' . $e->getMessage());
        }
    }
    
    /**
     * Renew all SSL certificates (SPRINT 11)
     */
    public function renewAllSSL()
    {
        try {
            // Execute certbot renew for all certificates
            $command = "certbot renew --nginx --quiet 2>&1";
            $output = shell_exec($command);
            
            return redirect()->back()
                ->with('success', 'All SSL certificates renewal process completed!');
            
        } catch (\Exception $e) {
            return redirect()->back()
                ->with('error', 'Failed to renew certificates: ' . $e->getMessage());
        }
    }
