...
class BackupsController extends Controller
{
    ...
    {
        $rawStats = $this->getBackupStats();
        $recentBackups = $this->getRecentBackups(10);
        
        // Map to view-expected keys (camelCase)
        $stats = [
            'totalBackups' => $rawStats['total_backups'] ?? 0,
            'totalSize' => $rawStats['total_size'] ?? '0 B',
            'lastBackup' => $rawStats['last_backup'] ?? 'Never',
            'nextScheduled' => 'Daily at 02:00' // Default value
        ];
    ...
    public function details(Request $request)
    {
        ...
        if (!$snapshotId) {
            return redirect()->route('backups.index')
                ->with('error', 'No snapshot ID provided');
        }
        
        $details = $this->getSnapshotDetails($snapshotId);
        
        if (!$details) {
            return redirect()->route('backups.index')
                ->with('error', 'Snapshot not found');
        }
        
        return view('backups.details', [
            'snapshot' => $details
        ]);
    }
