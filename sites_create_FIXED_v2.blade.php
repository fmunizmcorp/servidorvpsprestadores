<x-app-layout>
    ...
    <script>
        // SPRINT57 v2: CSRF token refresh with DOMContentLoaded
        console.log('SPRINT57 v2: Script loaded');
        
        document.addEventListener('DOMContentLoaded', function() {
            console.log('SPRINT57 v2: DOM ready, attaching event listener');
            
            const form = document.getElementById('site-create-form');
            if (!form) {
                console.error('SPRINT57 v2: Form not found!');
                return;
            }
            
            console.log('SPRINT57 v2: Form found, ID:', form.id);
            
            form.addEventListener('submit', function(e) {
                console.log('SPRINT57 v2: Form submit intercepted!');
                e.preventDefault();
                ...
                console.log('SPRINT57 v2: Fetching fresh CSRF token...');
                ...
                .then(response => {
                    console.log('SPRINT57 v2: Response status:', response.status);
                    if (!response.ok) {
                ...
                .then(data => {
                    console.log('SPRINT57 v2: Received fresh CSRF token');
                    
                    const csrfInput = form.querySelector('input[name="_token"]');
                    if (csrfInput && data.token) {
                        csrfInput.value = data.token;
                        console.log('SPRINT57 v2: CSRF token updated in form');
                    } else {
                        console.warn('SPRINT57 v2: Could not find CSRF input or token in response');
                    }
                ...
                .catch(error => {
                    console.error('SPRINT57 v2: Error refreshing CSRF token:', error);
            ...
            console.log('SPRINT57 v2: Event listener attached successfully');
        });
    </script>
</x-app-layout>
