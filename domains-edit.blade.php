        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            ...
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900">
                    <form method="POST" action="{{ route('email.domains.update', $domain->id) }}">
                        @csrf
                        @method('PUT')
                        ...
                        <div class="mb-4">
                            ...
                            <select name="status" class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700" required>
                                ...
                                <option value="inactive" {{ old('status', $domain->status) == 'inactive' ? 'selected' : '' }}>Inactive</option>
                            </select>
                            <p class="text-gray-600 text-xs mt-1">Set domain status (inactive domains won't receive emails)</p>
                            @error('status')
                                <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                            @enderror
                        </div>
                            ...
                            <div class="flex">
                                ...
                                <div class="ml-3">
                                    <p class="text-sm text-yellow-700">
                                        <strong>Warning:</strong> Changing the domain name will rename all system files and update all associated email accounts. 
                                        This is a critical operation. Please ensure the new domain name is correct.
                                    </p>
                                </div>
                            </div>
                        </div>
